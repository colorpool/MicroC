...
//        }
    ...
    private static int execCode(ArrayList<Integer> program, CubyBaseType[] stack, CubyBaseType[] inputArgs, boolean trace) throws ImcompatibleTypeError, OperatorError {
        ...
        for (;;) {
            ...
            switch (program.get(pc++)) {
                ...
                case Instruction.CSTB:
                    boolean rest;
                    if (program.get(pc++) == 0)
                        rest = false;
                    else
                        rest = true;
                    stack[sp + 1] = new CubyBoolType(rest); sp++; break;
                ...
                case Instruction.PRINTI: {
                    ...
                    if(stack[sp] instanceof CubyIntType){
                        ...
                        result = ((CubyFloatType)stack[sp]).getValue();
                    }else if (stack[sp] instanceof CubyBoolType){
                        result = ((CubyBoolType)stack[sp]).getValue()==false ?"false":"true";
                    }else {
                ...
                case Instruction.PRINTB:
                    Boolean restt=((CubyIntType)stack[sp]).getValue()==0?false:true;
                    System.out.print(restt+" "); break;
                    // if (stack[sp] instanceof CubyIntType==0) System.out.print("false");
                    // else System.out.print("true"); break;
    ...
    public static CubyBaseType binaryOperator(CubyBaseType lhs, CubyBaseType rhs, String operator) throws ImcompatibleTypeError, OperatorError {
        ...
        switch(operator){
            ...
            case ">>":{
                ...
                if (flag == 1) {
                    result = new CubyFloatType(Float.parseFloat(String.valueOf(left)) / (float) Math.pow(2,Float.parseFloat(String.valueOf(right))));
                } else {
                    result = new CubyIntType(Integer.parseInt(String.valueOf(left)) / (int) Math.pow(2,Integer.parseInt(String.valueOf(right))));
                }
                break;
            }
            case "<<":{
    ...
    private static String insName(ArrayList<Integer> program, int pc) {
        switch (program.get(pc)) {
            ...
            case Instruction.CSTS: return "CSTS " + program.get(pc+1);
            case Instruction.CSTB: return "CSTB " + program.get(pc+1);
            case Instruction.PRINTB: return "PRINTB";
            case Instruction.RSH: return "RSH";
            case Instruction.LSH: return "LSH";
